...
d_intv %>% select(!!!vector_weight) %>% as.matrix() %>% Hmisc::rcorr()
...
vars_iData <- 
  quos(
    ...
    t1text_assent1, 
    t1text_consent, 
    t1timecomp_writingessay, 
    t1timecomp_readingexpmaterials, 
    iHSGpa, 
    iHSPostal, 
    iHSCity, 
    iHSState, 
...
# Compute New Demographics Variables

## Disadvantaged Status and Race
...
d_all %<>%
  mutate(
    firstgen_race_num = 
      ifelse(
        race == "white" & firstgen == "not first-gen", 1,
        ifelse(
          race == "white" & firstgen == "first-gen", 2,
          ifelse(
            race == "asian" & firstgen == "not first-gen", 3,
            ifelse(
              race == "asian" & firstgen == "first-gen", 4,
              ifelse(
                race_asianmaj == "black/hispanic/native/pacificislander" & 
                  firstgen == "not first-gen", 5, 6
              )
            )
          )
        )
    ...
    firstgen_race = 
      factor(
        firstgen_race_num, 
        labels = c("white CG", "white FG", "asian CG", "asian FG", "minority CG", "minority FG")
      ),
    firstgen_race_sc = firstgen_race
  )
...
my.simple <- c - my.coding
# my.simple
contrasts(d_all$firstgen_race_sc) <- my.simple
d_all$firstgen_race_sc %>% attr("contrasts")

d_all %>% 
  demographics_pct(
    quos(
      race, firstgen, firstgen_race_sc, firstgen_race_num
      )
    )
```
