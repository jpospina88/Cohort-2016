...
```{r}
d_all %>% 
  filter(race == "white" & disadv == "disadv" & firstgen == "not first-gen")
```
...
```{r}
d_all %>% 
  filter(race_intl == "international student" & disadv == "adv" & firstgen == "first-gen")
```
...
d_all %<>%
  mutate(
    ...
    adv_hispanic = factor(adv_hispanic_num, labels = c("adv", "hispanic", "other")),
    adv_black_num = 
      ifelse(
        race == "black", 1,
        ifelse(
    ...
    adv_black = factor(adv_black_num, labels = c("adv", "black", "other")),
    ...
    adv_pi = factor(adv_pi_num, labels = c("adv", "pacific islander", "other")),
    adv_pi_sc = adv_pi
  )
...
d_all %>% 
  demographics_pct(quos(race, disadv, adv_hispanic, adv_black, adv_pi))

d_all %>% 
  demographics_pct(quos(race, disadv_sc, adv_hispanic_sc, adv_black_sc, adv_pi_sc))
```
