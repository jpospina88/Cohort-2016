...
d <- cv_compute(data = d, cv_name = belong_comp4, cv_vector = vector_belong2)

d %<>% mutate(success_usethis_percent = success_usethis*10)
d %<>% mutate(potential_usethis_percent = potential_usethis*10)
...
```{r list of variables}
# d %>% count(treat, treat_num) %>% select(-n) # If you have the package MASS loaded, this won't work.
...
vars_cont <- 
  quos(
    ...
    t1critfeedback_distinguishbtwnSs, # missing
    ## Feelings
    posfeels_comp4,  # missing
    t1posfeelings_profsTAs,  # missing
    t1posfeelings_gettingtoknowotherSs,  # missing
    t1posfeelings_beingawayfromhome, # missing
    t1posfeelings_receivingfeedback, # missing
    negfeels_comp4, # missing
    t1negfeelings_profsTAs, # missing
    t1negfeelings_gettingtoknowotherSs, # missing
    t1negfeelings_beingawayfromhome, # missing
    t1negfeelings_receivingfeedback,# missing
    ...
    health_behavbingedrink, # missing
    health_behavtobacco, # missing
    health_behavsleepiness, # missing
    stressdaily,
    pss_comp4,
    health_behavrelax,
    mhealthgeneral,
    ...
    bmi,
    bmi_chge_scr, # missing
    weight, # missing
    weight_comp3, # missing
    weight_happy_rc, # missing
    weight_feelbad, # missing
    weight_trylose, # missing
...
tribble_dv <- tribble(
  ...
  c(0, 100),      10,             "(Senior Year Survey)",                                3,       
  ...
  c(0, 40),       5,        "(BMI, Senior Year Survey)",                                 3,
  ...
  c(1, 5),        1.5,            "(Senior Year Survey)",                                3,
  "threat_nogen_comp8",           "Threat Composite",                                    seq(1, 6, 1),
  c(1, 6),        2,              "(8 Items; Senior Year Survey",                        3,
  ...
  c(1, 7),        2,              "(Senior Year Survey",                                 3,
  "stthreat_gender",              "Stereotype Threat: Gender",                           seq(1, 7, 1),
  c(1, 7),        2,              "(Senior Year Survey)",                                3, 
  "stthreat_race",                "Stereotype Threat: Race",                             seq(1, 7, 1),
  c(1, 7),        2,              "(Senior Year Survey)",                                3,
  "stthreat_ses",                 "Stereotype Threat: Social Class",                     seq(1, 7, 1), 
  c(1, 7),        2,              "(Senior Year Survey)",                                3, 
...
plots <- mapply(plot_raincloud_map_treat_adv, # function
       ...
       tribble_comb[,"limit"] %>% t, tribble_comb[,"subheader"] %>% t, 
       SIMPLIFY = FALSE
       )
...
theme_cond <- 
  theme(
    ...
    axis.title = element_text(size = 22, family = "Helvetica"), 
    strip.text.x = element_text(size = 15, colour = "white", family = "Helvetica"),
    plot.title = element_text(size = 22, hjust = 0.5),
    plot.subtitle = element_text(size = 16, hjust = 0.5),
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(),
...
mapply(plot_raincloud_map_cond_adv, # function
       tribble_comb[,"cond_sc"] %>% t, tribble_comb[,"dv"] %>% t, # arguments
       tribble_comb[,"dv_name"] %>% t, tribble_comb[,"scale"] %>% t, 
       tribble_comb[,"limit"] %>% t, SIMPLIFY = F)
```
...
cohend_ineq <- 
  cohen.d.by(x = d %>% 
               select(treat_num, disadv_num, !!!vars_cont), 
             group = "disadv_num", 
             group2 = "treat_num", 
             alpha = .05) 
...
```{r}
cohend_disadv_all <- cohen.d(d %>% select(disadv_num, !!!vars_cont), group = "disadv_num", alpha = .05)
...
```{r}
cohend_treat_all <- cohen.d(d %>% select(treat_num, !!!vars_cont), group = "treat_num", alpha = .05)
...
cohend_treat <- cohen.d.by(x = d %>% select(treat_num, disadv_num, !!!vars_cont), group = "treat_num", group2 = "disadv_num", alpha = .05)
...
```{r}
cohend_bel <- cohen.d.by(x = d %>% select(cd_bel_num, disadv_num, !!!vars_cont), group = "cd_bel_num", group2 = "disadv_num", alpha = .05)
...
#### Culture
```{r}
cohend_cul <- cohen.d.by(x = d %>% select(cd_cul_num, disadv_num, !!!vars_cont), group = "cd_cul_num", group2 = "disadv_num", alpha = .05) 
...
#### Wise Feedback
```{r}
cohend_wf <- cohen.d.by(x = d %>% select(cd_wf_num, disadv_num, !!!vars_cont), group = "cd_wf_num", group2 = "disadv_num", alpha = .05) 
...
## Example
```{r}
lm(cum_gpa_y4_spring ~ treat_sc*disadv_sc, data = d) %>% summary
```
...
dvs_col_rep %>% 
  ...
  round_table(quos(dv_name, contrast, disadv_sc), 2) %>% 
  kable_format
```
...
plots <- mapply(plot_bar_x_y_group_treat_adv, # function
       ...
       tribble_comb[,"decimal"] %>% t, tribble_comb[,"subheader"] %>% t,
       SIMPLIFY = F)
...
plot_bar_x_y_group_cond_adv <- function(ftr, var, y_lab, scales, limit, position, decimal, subheader) d.unnest_cond %>% 
  ...
  ggplot(aes(x = get(ftr), y = get(var), color = get(ftr), group = get(ftr))) + # Remember to add get()
  # geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)), size = 42, color = "black") +
  geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)), size = 24) +
  geom_errorbar(aes(ymin = get(var) - SE, ymax = get(var) + SE), width = .4, position = pd2, color = "black") +
  theme1 +
...
## Example
```{r}
lm(cum_gpa_y4_spring ~ treat_sc*disadv_sc + gender + sattoact + hsrank + hsrank_no, data = d) %>% summary
```
