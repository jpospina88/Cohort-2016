...
## Example
```{r}
glm(major_grad_4y ~ treat_sc*disadv_sc, data = d, family = binomial, na.action = na.omit) %>% 
  summary
```
...
glm_model <- function(data){
  glm(dv_value ~ treat_sc*disadv_sc, data = data, family = binomial, na.action = na.omit)
}

glm_model_cond <- function(data){
  glm(dv_value ~ cond_sc*disadv_sc, data = data, family = binomial, na.action = na.omit)
}
...
plot_bar_x_y_group_treat_adv <- function(ftr, var, y_lab, scales, limit,
  ...
  geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)*100), size = 41) +
...
plot_bar_x_y_group_cond_adv <- function(ftr, var, y_lab, scales, limit,
  ...
  ggplot(aes(x = get(ftr), y = get(var), color = get(ftr), group = get(ftr))) + # Remember to add get()
  # geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)), size = 42, color = "black") +
  geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)*100), size = 23) +
  # geom_errorbar(aes(ymin = get(var) - SE, ymax = get(var) + SE), width = .4, position = pd2, color = "black") +
  theme1 +
  # geom_text(aes(label = format(get(var)*100, digits = 4), y = 10), size = 8, color = "black") +
  geom_text(aes(label = format(paste0(round(get(var)*100, digits = 1), "%"), digits = 3), y = position), 
...
plot_bar_x_y_group_cond_adv <- function(ftr, var, y_lab, scales, limit) d.unnest_cond %>% 
  ...
  geom_bar(position = pd1, stat = "identity") + 
  theme1 + guides_no + 
  geom_text(aes(label = format(paste0(round(get(var)*100, digits = 1), "%"), digits = 3), y = 5)) +
  coord_cartesian(ylim = limit) +
  scale_y_continuous(breaks = scales) +
  facet_grid(, vars(disadv_sc), labeller = as_labeller(adv_names)) + # ignore warning, this is to make the plot facet the variables in the way that we want it
...
mapply(plot_bar_x_y_group_cond_adv, # function
       tribble_categ[,"cond_sc"] %>% t, tribble_categ[,"dv"] %>% t, # arguments
       tribble_categ[,"dv_name"] %>% t, tribble_categ[,"scale"] %>% t, 
       tribble_categ[,"limit"] %>% t, SIMPLIFY = F)
```
...
## Example
```{r}
glm(major_grad_4y ~ treat_sc*disadv_sc + gender + sattoact + hsrank + hsrank_no, 
...
glm_model_treat_cv <- function(data){
  glm(dv_value ~ treat_sc*disadv_sc + gender + sattoact + hsrank + hsrank_no, 
    data = data, family = binomial, na.action = na.omit)
  
}

glm_model_cond_cv <- function(data){
  glm(dv_value ~ cond_sc*disadv_sc + gender + sattoact + hsrank + hsrank_no, 
    data = data, family = binomial, na.action = na.omit)
}
