...
## Example
```{r}
MASS::glm.nb(visits_per_day ~ treat_sc * firstgen_race_sc, data = d) %>% 
  summary
```
...
#Transforming negative binomial coefficients to incidence rate ratios without confidence intervals
coef.irr <- function(model) {
...
model_z <- coef(summary(model))[, 3]
model_p <- coef(summary(model))[, 4]
ratio_est <- cbind(coef_exp, irr_se, model_z, model_p
                   # , irr_confint
                   )
colnames(ratio_est) <- c("IRR (Ratio)", "IRR.SE", "statistic", "p.value"
                         # , "LL 2.5%", "UL 97.5%"
                         )
...
glm_model <- function(data){
  MASS::glm.nb(dv_value ~ treat_sc * firstgen_race_sc, data = data)
}

glm_model_cond <- function(data){
  MASS::glm.nb(dv_value ~ cond_sc * firstgen_race_sc, data = data)
}
...
dvs_col_rep <- dvs_col$dv_name %>% 
  rep(each = 22) %>% # change this number depending on the number of rows of your tidy datasets
  as.tibble %>% 
  rename(dv_name = value)
...
plot_bar_x_y_group_treat_adv <- function(ftr, var, y_lab, scales, limit,
  ...
  geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)), size = 14) +
  ...
  geom_text(aes(label = format(paste0(round(get(var), digits = 1)), digits = 1), y = position), 
            size = 5, color = "black") +
...
plot_bar_x_y_group_cond_adv <- function(ftr, var, y_lab, scales, limit,
  ...
  ggplot(aes(x = get(ftr), y = get(var), color = get(ftr), group = get(ftr))) + # Remember to add get()
  # geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)), size = 42, color = "black") +
  geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)), size = 7.5) +
  geom_errorbar(aes(ymin = get(var) - SE, ymax = get(var) + SE), width = .4, position = pd2, color = "black") +
  theme1 +
  # geom_text(aes(label = format(get(var)*100, digits = 4), y = 10), size = 8, color = "black") +
  geom_text(aes(label = format(paste0(round(get(var), digits = 1)), digits = 1), y = position), 
...
## Example
```{r}
MASS::glm.nb(visits_per_day ~ treat_sc * firstgen_race_sc + gender + sattoact + hsrank + hsrank_no, data = d) %>% 
  summary
```

```{r}
glm_model_treat_cv <- function(data){
  MASS::glm.nb(dv_value ~ treat_sc * firstgen_race_sc + gender + sattoact + hsrank + hsrank_no, data = data)
}

glm_model_cond_cv <- function(data){
  MASS::glm.nb(dv_value ~ cond_sc * firstgen_race_sc + gender + sattoact + hsrank + hsrank_no, data = data)
}
...
dvs_col_rep <- dvs_col$dv_name %>% 
  rep(each = 26) %>% # change this number depending on the number of rows of your tidy datasets
  as.tibble %>% 
  rename(dv_name = value)
