...
```{r}
treat_adv <- list(d$treat_sc, d$firstgen_race_sc)
...
```{r}
cond_adv <- list(d$cond_sc, d$firstgen_race_sc)
...
## Example
```{r}
glm(major_grad_4y ~ treat_sc*firstgen_race_sc, data = d, family = binomial, na.action = na.omit) %>% 
  summary
```
...
```{r}
vars_ivs <- quos(idnum, firstgen_race_sc, treat_sc, cond_sc,
...
glm_model <- function(data){
  glm(dv_value ~ treat_sc*firstgen_race_sc, data = data, family = binomial, na.action = na.omit)
}

glm_model_cond <- function(data){
  glm(dv_value ~ cond_sc*firstgen_race_sc, data = data, family = binomial, na.action = na.omit)
}
...
d.nest %<>% 
  mutate(
    ...
    emmean_ineq_treat = pmap( # Inequality by Treatment
      .l = list(
        object = glm_model, 
        specs = "firstgen_race_sc", 
        by = "treat_sc"
      ),
      .f = emmeans,
      type = "response"
    ...
    emmean_ineq_treat_b = pmap( # Inequality by Treatment
      .l = list(
        object = glm_model, 
        specs = "firstgen_race_sc", 
        by = "treat_sc"
    ...
    emmean_ineq_cond = pmap( # Inequality by Condition
      .l = list(
        object = glm_model_cond, 
        specs = "firstgen_race_sc", 
        by = "cond_sc"
    ...
    emmean_ineq_cond_b = pmap( # Inequality by Condition (beta)
      .l = list(
        object = glm_model_cond, 
        specs = "firstgen_race_sc", 
        by = "cond_sc"
...
dvs_col_rep %>% 
  ...
  round_table(quos(dv_name, contrast, firstgen_race_sc), 2) %>% 
  kable_format
```
...
dvs_col_rep <- dvs_col$dv_name %>% 
  rep(each = 60) %>% # change this number depending on the number of rows of your tidy datasets
  as.tibble %>% 
  rename(dv_name = value)
...
dvs_col_rep <- dvs_col$dv_name %>% 
  rep(each = 36) %>% # change this number depending on the number of rows of your tidy datasets
  as.tibble %>% 
  rename(dv_name = value)
...
plot_bar_x_y_group_treat_adv <- function(ftr, var, y_lab, scales, limit,
  ...
  geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)*100), size = 14) +
...
theme_cond <- 
  theme(
    ...
    strip.text.x = element_text(size = 12, colour = "white", family = "Helvetica"), # change size of factor labels 
...
plot_bar_x_y_group_cond_adv <- function(ftr, var, y_lab, scales, limit,
  ...
  ggplot(aes(x = get(ftr), y = get(var), color = get(ftr), group = get(ftr))) + # Remember to add get()
  # geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)), size = 42, color = "black") +
  geom_segment(aes(x = get(ftr), xend = get(ftr), y = -10, yend = get(var)*100), size = 7.5) +
  # geom_errorbar(aes(ymin = get(var) - SE, ymax = get(var) + SE), width = .4, position = pd2, color = "black") +
  theme1 +
  # geom_text(aes(label = format(get(var)*100, digits = 4), y = 10), size = 8, color = "black") +
  geom_text(aes(label = format(paste0(round(get(var)*100, digits = 1), ""), digits = 3), y = position), 
...
## Example
```{r}
glm(major_grad_4y ~ treat_sc*firstgen_race_sc + gender + sattoact + hsrank + hsrank_no, 
...
glm_model_treat_cv <- function(data){
  glm(dv_value ~ treat_sc*firstgen_race_sc + gender + sattoact + hsrank + hsrank_no, 
    data = data, family = binomial, na.action = na.omit)
  
}

glm_model_cond_cv <- function(data){
  glm(dv_value ~ cond_sc*firstgen_race_sc + gender + sattoact + hsrank + hsrank_no, 
    data = data, family = binomial, na.action = na.omit)
}
...
dvs_col_rep <- dvs_col$dv_name %>% 
  rep(each = 16) %>% # change this number depending on the number of rows of your tidy datasets
  as.tibble %>% 
  rename(dv_name = value)
...
dvs_col_rep <- dvs_col$dv_name %>% 
  rep(each = 28) %>% # change this number depending on the number of rows of your tidy datasets
  as.tibble %>% 
  rename(dv_name = value)
